...
const MenuItemWithChildren = ({ item, linkClassName, subMenuClassNames, activeMenuItems, toggleMenu, }: SubMenus) => {
  ...
  return (
    <li className={`side-nav-item ${open ? 'menuitem-active' : ''}`}>
      <Link
        to='#'
        className={`side-nav-link ${linkClassName} ${{ 'menuitem-active': activeMenuItems!.includes(item.key) ? 'active' : '' }}`}
        aria-expanded={open}
      ...
      if (matchingMenuItem) {
        ...
        setTimeout(function () {
          ...
          if (activatedItem != null) {
            var simplebarContent = document.querySelector('#leftside-menu-container .simplebar-content-wrapper');
        ...
        {(menuItems || []).map((item, idx) => {
          return (
            <React.Fragment key={idx}>
              {item.isTitle ? (<li className='side-nav-title'>
                {item.label}
              </li>) : (<>
                {item.children ? (
                  <MenuItemWithChildren
