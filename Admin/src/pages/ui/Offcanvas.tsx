import { Button, Card, Col, Row, Offcanvas as BootstrapOffcanvas } from 'react-bootstrap';
...
const OffCanvasWithBackdrop = ({ name, ...props }: BackdropOption) => {
  ...
  return (
    <>
      <Button variant='primary' onClick={toggle} className="mt-2 me-2 mt-md-0" >{name}</Button>
...
const OffcanvasPlacement = ({ name, ...props }: PlacementOption) => {
  ...
  return (
    <>
      <Button variant='primary' onClick={toggle} className="mt-2 me-2 mt-md-0"> Toggle {name} offcanvas</Button>
    ...
    <Card>
      <Card.Body>
        ...
        <BootstrapOffcanvas show={isOpen} onHide={toggle} className="offcanvas-start" tabIndex={-1} >
          <BootstrapOffcanvas.Header closeButton>
            <BootstrapOffcanvas.Title as="h5">Offcanvas</BootstrapOffcanvas.Title>
          </BootstrapOffcanvas.Header>
          <BootstrapOffcanvas.Body className="offcanvas-body">
            <div>
              Some text as placeholder. In real life you can have the elements you have chosen. Like, text,
  ...
  return (
    <Card>
      <Card.Body>
        ...
        <p className="text-muted fs-13">Change the appearance of offcanvases with utilities to better match them to different contexts like dark navbars.
          Here we add <code>.text-bg-dark</code> to the <code>.offcanvas</code> and <code>.btn-close-white</code> to <code>.btn-close</code> for proper styling with a dark offcanvas. If you have dropdowns within, consider also adding <code>.dropdown-menu-dark</code> to <code>.dropdown-menu</code>.</p>

        <Button variant='primary' onClick={toggle} className="mt-2 mt-md-0">Dark offcanvas</Button>

        <BootstrapOffcanvas show={isOpen} onHide={toggle} className="offcanvas-start text-bg-dark">
          <BootstrapOffcanvas.Header closeButton>
            <BootstrapOffcanvas.Title as="h5">Dark Offcanvas</BootstrapOffcanvas.Title>
          </BootstrapOffcanvas.Header>
