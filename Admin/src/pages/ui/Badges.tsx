    ...
    <>
      <Card>
        <Card.Body>
          ...
          <h1>h1.Example heading <Badge bg='secondary' className="text-light">New</Badge></h1>
          <h2>h2.Example heading <Badge bg='success' className="bg-success-subtle text-success">New</Badge></h2>
          <h3>h2.Example heading <Badge bg='primary'>New</Badge></h3>
          <h4>h4.Example heading <Link to="#" className="badge bg-info-subtle text-info">Info Link</Link></h4>
          <h5>h5.Example heading <Badge bg='' className="badge-outline-warning">New</Badge></h5>
          <h6>h6.Example heading <Badge bg='danger'>New</Badge></h6>
        </Card.Body>
      </Card>
    </>
  )
        ...
        <Card.Body>
          ...
          <div className='d-flex flex-wrap gap-1'>
            {(colors || []).map((color, idx) => {
              return (
                <Badge key={idx} pill bg={color}>{color.charAt(0).toUpperCase() + color.slice(1)}</Badge>
              )
            })
            }
          </div>
          ...
          <div className='d-flex flex-wrap gap-1'>
            {(colors || []).map((color, idx) => {
              return (
                <Badge key={idx} pill className={`bg-${color}-subtle text-${color}`}>{color.charAt(0).toUpperCase() + color.slice(1)}</Badge>
              )
            })
            }
          </div>
          ...
          <div className='d-flex flex-wrap gap-1'>
            {(colors || []).map((color, idx) => {
              return (
                <Badge key={idx} pill bg='' className={`badge-outline-${color}`}>{color.charAt(0).toUpperCase() + color.slice(1)}</Badge>
              )
            })
            }
          </div>
    ...
    <Card>
      <Card.Body>
        ...
        <div className='d-flex flex-wrap gap-1'>
          {(colors || []).map((color, idx) => {
            return (
              <Badge key={idx} bg={color}>{color.charAt(0).toUpperCase() + color.slice(1)}</Badge>
            )
          })
          }
        </div>
        ...
        <div className='d-flex flex-wrap gap-1'>
          {(colors || []).map((color, idx) => {
            return (
              <Badge key={idx} className={`bg-${color}-subtle text-${color}`}>{color}</Badge>
            )
          })
          }
        </div>
        ...
        <div className='d-flex flex-wrap gap-1'>
          {(colors || []).map((color, idx) => {
            return (
              <Badge key={idx} bg='' className={`badge-outline-${color}`}>{color}</Badge>
            )
          })
          }
        </div>
    ...
    <>
      ...
      <Col xs={6}>
        <Button variant='success' className="mt-4">
          Notifications <Badge className="bg-light text-dark ms-1">4</Badge>
        </Button>
      </Col>
    </>
  )
